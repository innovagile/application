    ...
    is_accrued_allowance : {
      ...
      classMethods: {
          loadScope : function( models ) {
            ...
            Department.addScope(
              'with_secondary_users',
              {
                include : [
                  { model : models.User, as : 'secondary_users' },
                ]
              }
            );
          },
          associate : function( models ) {
            ...
            Department.belongsToMany( models.User, {
              ...
              foreignKey : 'department_id',
              otherKey   : 'user_id',
              through    : models.UserSecondaryDepartment,
            });
          },
      ...
      instanceMethods : {
        ...
        promise_active_users : function(){
          // TODO VPP: add users from secondary departmenst
          return this.getUsers({
            where : sequelize.models.User.get_active_user_filter()
          });

          // Following code is experemental

          let self = this;

          return Promise.join(
            self.getUsers(),
            sequelize.models.UserSecondaryDepartment
              .findAll({ where : { department_id : self.id } }),
            (users, usds) => {
              let ids = [];

              users.forEach(u => ids.push(u.id));

              usds.forEach(r => ids.push(r.user_id));

              return sequelize.models.User.findAll({
                where : {
                  $and : [
                    { id : ids },
                    sequelize.models.User.get_active_user_filter(),
                  ]
                }
              });
            }
        ...
        promise_me_with_secondary_users : function() {
          let self = this;

          return self.Model.scope('with_secondary_users').findById( self.id );
        },
      }
  });
